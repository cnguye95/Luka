...
- **§6.2's missing-derivative rule becomes a lookup.** Discovery no longer guesses `<stem>.md` from the source path and parses the file's `derived-from` to see whose it is; it reads the recorded pointer and asks whether that file still exists. This **supersedes** M2d's "Checking derivative *ownership* rather than existence costs a read and a YAML parse per unchanged converting source per compile", which is now zero reads and zero parses. The ownership check existed to defend against a *guessed* path — a stranger's file sharing a stem — and there is no longer a guess to defend. The residual case is a user overwriting a derivative in place and stripping `derived-from`. The source then reads as unchanged rather than re-extracting, while the stripped file becomes a source in its own right. It does not go silent: the guard below still refuses to serve that file as the source's body, so any page citing the source reports "no readable markdown" until it is put right, and the next edit to the source fails its re-extraction against the same guard. It is narrower than what a guessed path exposed, where any unrelated file sharing a stem could trigger it.
- **A renamed source's entry takes its pointer from discovery's classification, not from the old entry.** A `move` carry leaves the derivative at the new path, so copying the old entry whole would record a pointer to a file that is no longer there and re-extract over a hand repair on the next compile — at the model call the rename exists to avoid.
- **A re-extracting rename records no pointer until the replacement exists.** The entry written before normalization says only "this hash, no derivative", so a run that dies between the two leaves a source that still reads as owed one. Writing the intended location early would have a failure read as fully ingested.
...
- **A rename is no longer counted as `modified` when its derivative cannot be carried.** It is one event, and §6.2 calls it a rename. Two consequences, both accepted. A pure-rename diff whose carry hits a complication no longer opens the §8.1 confirm modal — correctly, since nothing page-destructive happens on that path: no page is requeued and no page is deleted. And pages citing that source are not regenerated, which is right, because the source's content is identical — that is how the rename was detected.
...
- **A pre-ownership entry leaves its old derivative behind.** A manifest written before ownership was recorded names no file, so a source that renames or is deleted under such an entry has nothing for the sweep to act on, and the markdown at its old stem stays as an unreferenced `.md`. Finding it would mean guessing `<stem>.md` — the inference invariant II exists to remove — for the sake of a migration with no instances in the wild. It is invisible rather than harmful: a file carrying `derived-from` is never a source, so it neither re-ingests nor cites anything.
- **A leftover the carry could not delete is reported once, not chased.** The rename completed, so its entry names the new derivative; keeping the old one reachable would need a second pointer per entry and a retry path for it, which is the extra failure state this milestone exists to remove. This supersedes M2d's **"restores the old path of a rename whose stale derivative could not be swept"** — that restore made the *rename* re-present in order to retry a *file deletion*, which is why it also had to defend against the restored entry pairing with an unrelated file.
